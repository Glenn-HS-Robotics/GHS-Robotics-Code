...
@Suppress("unused")
@TeleOp(name = "Basic: Omni Linear OpMode Encoder BASE", group = "Linear OpMode")
class BasicOmniOpModeEncoder_Linear_Base : LinearOpMode() {
    ...
    override fun runOpMode() {
        ...
        hoodServoLeft!!.position = MAX_HOOD
        hoodServoRight!!.position = MAX_HOOD
    ...
    fun runBot(){
        ...
        yaw = getAprilTagAdjustedYaw(yaw)
    ...
    fun getAprilTagAdjustedYaw(currentYaw: Double): Double {
        var yaw = currentYaw
        ...
        if (target != null && target.ftcPose != null) {
            ...
            telemetry.addData(
                "Pose", "yawRaw=%.1f yawF=%.1f y=%.1f z=%.1f",
    ...
    private fun manageHood(){
        ...
        val angle = MIN_HOOD + (hoodHoodedness) * (MAX_HOOD - MIN_HOOD)
